...
func newAuthorization(version string) *authorizeModel {
	...
	appID.CharLimit = 64
	appID.SetValue(defaultAuthorizeAppID)
	...
	deviceName.SetValue(defaultAuthorizeDeviceName)
...
func runAuthorize(version string) error {
	_, err := tea.NewProgram(newAuthorization(version)).Run()
	return err
}
...
type authorizeModel struct {
	...
	appID      textinput.Model
	deviceName textinput.Model
...
func (m *authorizeModel) Init() tea.Cmd {
	return textinput.Blink
}

func (m *authorizeModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	var cmd tea.Cmd
...
func (m *authorizeModel) View() string {
	var view string
	...
	if m.err != nil {
		view += "\n" +
			lipgloss.NewStyle().Bold(true).Foreground(lipgloss.Color("#FF0000")).Render("× "+m.err.Error())
		return view + "\n"
	}
